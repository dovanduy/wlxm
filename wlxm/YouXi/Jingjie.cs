    ...
    public class Jingjie:youxi
    {
            ...
            while (true)
            {
                ...
                if (xuanqu == 0 && mf.mohuByLeiBool(yk.Sd))
                {
                    ...
                    while (true)
                    {
                        ...
                        if (xuanqu == 0 && mf.mohuByLeiBool(yk.Sd))
                        {
                            ...
                            mf.mydelay(300, 600);
                            mf.mytap(this._jubing, 438, 136);//点击清空按钮
                            mf.mydelay(300, 600);
                            ...
                            mf.mydelay(300, 600);
                            mf.mytap(this._jubing, 438, 175);//点击清空按钮
                            mf.mydelay(2000, 4000);
                ...
                if (xuanqu == 1 && xuanhao == 0 && t0)
                {
                    ...
                    if (mf.mohuByLeiBool(xq.Sd))
                    {
                        ...
                        mf.mytap(this._jubing, 84, 178);//增加新区要改
                    }
                }
        ...
        public Boolean denglu(int fenzhong,ref string name,ref string pwd)
        {
            ...
            while (true) {
                ...
                {
                    WriteLog.WriteLogFile(this._mnqName, "搞登录,取到点" + rr);
                    tiaochu4 = 1;
                    break;
                }
                if (tiaochu4 == 0 && (jsp - ksp) > 1000 * 10 && (zhuxianrt != null && zhuxianrt.Length > 0))
                {
                    tiaochu4 = 1;
                    WriteLog.WriteLogFile(this._mnqName, "不搞登录,取到点" + yijinruzhuxian);
                    break;
                }
            }
            ...
            while (true)
            {
                ...
                if (denglu==0 && mf.mohuByLeiBool(kt.Sd))
                {
                    ...
                    mf.mytap(this._jubing, 438, 174);//点击清空按钮
                    mf.mydelay(300, 600);
                    mf.mytap(this._jubing, 373, 171);//点击密码
                    mf.mydelay(4000, 6000);
                    if (mf.mohu(504, 27, 0xffffff) == 1)
                    {
                        WriteLog.WriteLogFile(this._mnqName, "搞密码");
                ...
                kt = Jingjie_SanDian.GetObject().findFuHeSandianByName("登录-登录账号2");
                if (denglu == 0 && mf.mohuByLeiBool(kt.Sd))
                {
                    WriteLog.WriteLogFile(this._mnqName, kt.Name);
                    mf.mytap(this._jubing, 309, 138);
                    mf.mydelay(4000, 6000);
                    if (mf.mohu(504, 27, 0xffffff) == 1)
                    {
                        WriteLog.WriteLogFile(this._mnqName, "搞账号");
                    ...
                    if (mf.mohu(368, 229, 0x1daddf) == 1)
                    {
                        WriteLog.WriteLogFile(this._mnqName, "搞登录");
                        mf.mydelay(2000, 4000);
                        mf.mytap(this._jubing, 343, 228);//点击注册
                        mf.mydelay(9000, 18000);
                        if (!mf.mohuXunHuanJianChi(kt.Sd, 20))
                        {
                            denglu = -1;
                ...
                kt = Jingjie_SanDian.GetObject().findFuHeSandianByName("登录-选择新账号");
                if (mf.mohuByLeiBool(kt.Sd))
                {
                    WriteLog.WriteLogFile(this._mnqName, kt.Name);
                    mf.mytap(this._jubing, kt.Zhidingx, kt.Zhidingy);
                ...
                FuHeSanDian dlxf = Jingjie_SanDian.GetObject().findFuHeSandianByName("登录-进入游戏");
            ...
            while (true)
            {
                ...
                if (meirijiangli == 0 && mf.mohuXunHuanJianChi(ktsd1.Sd, 20))
                {
                     ...
                     if (meirijiangli == 0)
                     {
                         ...
                         ktsd1 = Jingjie_SanDian.GetObject().findFuHeSandianByName("特殊引导-1天送礼");
                         if (meirijiangli == 0 && mf.mohuByLeiBool(ktsd1.Sd))
                         {
                             compareSandianAndtap(ktsd1, 2000, () =>
                             {
                         ...
                         ktsd1 = Jingjie_SanDian.GetObject().findFuHeSandianByName("特殊引导-1天送礼2");
                         if (meirijiangli == 0 && mf.mohuByLeiBool(ktsd1.Sd))
                         {
                             compareSandianAndtap(ktsd1, 2000, () =>
                             {
                ...
                if (meirijiangli == 1 && mf.mohuByLeiBool(ktsd1.Sd))
                {
                    mf.mytap(this._jubing, 633, 40);
                }
                ktsd1 = Jingjie_SanDian.GetObject().findFuHeSandianByName("特殊引导-第3天送啥");
                ...
                ktsd1 = Jingjie_SanDian.GetObject().findFuHeSandianByName("特殊引导-上阵人数不足2");
                if ((zdrs5 == 0) && mf.mohuByLeiBool(ktsd1.Sd))
                {
                    WriteLog.WriteLogFile(this._mnqName, ktsd1.Name);
                    mf.mytap(this._jubing, ktsd1.Zhidingx, ktsd1.Zhidingy);
        ...
        public void quitdq(string name)
        {
            ...
            if (mf.mohuXunHuanJianChi(fh.Sd, 15))
            {
                ...
                if (mf.mohuByLeiBool(fh.Sd))
                {
                    ...
                    if (mf.mohu(235, 319, 0x31b1a1) == 1 || mf.mohu(234,  319, 0x44b8af)==1)
                    {
        ...
        public void qianghua()
        {
            ...
            foreach(int ij in juesel){
                ...
                if (panduanjiemian("界面-角色强化")) {
                    mf.mytap(this._jubing, 40, 57+(ij-1)*55);
                    mf.mydelay(2000, 3000);
