            ...
            lock (obj)
            {
                try
                {
                    ...
                    foreach(string pc in pcnames){
                        selsql = selsql + "sum(case when  z.pcname='"+pc+"' and z.xgsj>=convert(varchar(10),getdate(),120) then 1 else 0 end) "+pc +",";
                    }
                    selsql = selsql + "z.sum(dqinx) wuyiyi  from zhanghao z where yxbz='Y' and yimai='N'";  
